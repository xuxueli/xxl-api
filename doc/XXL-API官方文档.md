...
- 3、权限控制：支持以业务线为维度进行用户权限控制，分配权限才允许操作业务线下项目接口和数据类型，否则只允许查看；
- 4、项目内API搜索关键字改为URL，更加贴合用户需求；
- 5、设计实现API "响应数据类型" 模块，"响应数据类型"不仅可以被多个API复用，而且支持数据类型嵌套；
- 6、"数据类型"功能：系统支持录入数据类型，数据类型支持嵌套，每个API只需要绑定一个数据类型，不需要单独执行响应数据参数； 
- 7、项目maven依赖升级；
- 8、UI交互优化，列表自适应性优化；
- 9、底层代码重构；
