...
### 2.4 部署
将应用 "xxl-api-admin" 部署在容器如 Tomcat 下之后，即可访问：
...
### 8.3 版本 V1.1.1 特性[迭代中]
- 1、接口"成功/失败响应结果"JSON格式华支持，方便数据查看；
- 2、项目迁移至 springboot 版本；
- 3、项目下存在接口时拒绝删除，防止数据误删除；
- 3、【ING】API导出为PDF； 
- 4、接口分组删除问题修复；
- 5、接口在线测试功能对于响应状态码为302的请求未能正确展示“Location”问题修复；
...
- 17、支持swagger、postman等格式接口数据导入；
- 18、简化测试页，支持Select方式选择接口，自动填充接口数据并测试；
- 19、提供 docker 镜像；
- 20、接口绑定数据类型时，支持指定标准格式（code、msg、data）；支持自动生成完整数据接口；
- 21、接口数据类型，自动生成Mock数据，提供Runpath接口根据接口地址自动匹配接口生成Mock数据；
- 22、可配置参数支持拖动改变顺序；
