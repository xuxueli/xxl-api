...
- 14、支持Restful类型接口，如 GET /job/{id}/detail ，其中{id}在接口测试时支持动态变化；
- 15、接口历史版本功能，记录接口操作变更历史；
- 16、支持 https 类型接口在线测试，处理接口证书问题；
- 17、请求头部，除了支持下拉框外，支持手动填写；
- 18、支持swagger、postman等格式接口数据导入；
- 19、简化测试页，支持Select方式选择接口，自动填充接口数据并测试；
- 20、项目迁移至 springboot 并提供 docker 镜像；
